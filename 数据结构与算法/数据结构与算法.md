...
- 如果$\sum^{k}_{i=1} a_i <1$，则方程$T(N)=\sum^{k}_{i=1}T(a_iN)+O(N)$的解为$T(N)=O(N)$；
...
- **线性同余数发生器**（最简单的随机数生成器）
  ...
  - 为了获得足够长的周期，建议使用31比特素数$M=2^{31}-1=2147483647,A=48271$;
...
- **费马小定理**
  ...
  - 如果$2^{N-1}\equiv 1(modN)$不成立，那么$N$肯定不是素数；
  - 满足$2^{N-1}\equiv 1(modN)$但不是素数的最小$N$值为341；
  - 存在一个固定集合$U \in 0<P<N$，使得该定理不成立，因此可以采用随机算法对其进行验证；

- 定理二

  - 如果$P$为素数且$0<X<P$，那么$X^2 \equiv 1(modP)$仅有两个解为$X=1,P-1$。

  - 该公式可以推导为：
    $$
    X^2 \equiv 1(modP) => X^2 -1 \equiv 0(modP) => (X -1)(X + 1) \equiv 0(modP)
    $$

  - 由于$P$ 为素数，对于任意$0<X<P$，P必然能够整除$(X-1)$或者$(X+1)$；
...
- 进行不同的尝试，在发现不符合当前定义时，对当前或之前的操作进行删除的算法；
- 在一步内删除一大组可能性的做法叫做**裁剪**；

#### (2). 收费公路重建问题

- **点集**：
  - 给定N个位于$x-$轴上的点$p_1, p_2, p_3,...,p_N$，$x_i$为$p_i$点的$x$坐标；
  - 假设$x_0=0$，同时这些点按照从左到右进行排布；
- **距离集合**：
  - $N$个点确定在每一对点间的$N(N-1)/2$个（不唯一）形如$|x_i-x_j|(i \neq j)$的距离；
- **收费公路重建问题**：
  - 从**距离集合**中重新构造出**点集**；
...
- 极小极大策略
  ...
  - 通过考察盘面就能够确定输赢的位置叫做**终端位置**；

  - **极小极大策略**：通过递归假设双方最优棋步确定**非终端位置**的策略；

  - 位置$P$的**后继位置**：通过从位置$P$走一步棋可以达到的任意位置$P_s$；

  - $P$的值为计算机选择的具有最大值的一步行棋，为了得到任意后继位置$P_s$的值，要递归地算出$P_s$的所有后继位置的值，然后选取其中最小的值；
